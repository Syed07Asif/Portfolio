...
- `utils.ts` — generic helpers with no framework or content coupling. Currently just `cn()` (clsx + a tailwind-merge configured with our custom `--text-*` font-size scale — see its own comment for why that config matters), used by every primitive in `components/ui` and by shadcn/ui.
